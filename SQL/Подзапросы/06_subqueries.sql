...
Выбираются топ-10 самых доставляемых товаров, результат сортируется по количеству доставок по убыванию.
*/
with sub1 as (SELECT order_id,
                     unnest(product_ids) as product_id
              FROM   orders), sub2 as (SELECT DISTINCT order_id,
                                         product_id
                         FROM   sub1)
SELECT name,
